...
class CategoryEmptyViewController: UIViewController {
    ...
    private let messageLabel: UILabel = {
        let label = UILabel()
        let text = """
        카테고리를 추가하고 스냅팝 기능을 경험해보세요!
        스냅 사진으로 진행사항을 쉽게 확인하고,
        제품 비용 통계를 한 눈에 파악 할 수 있습니다.
        """
   ...
   @objc private func handleCategoryEmptycheck() {
    ...
    @objc private func actionButtonTapped() {
        guard let viewModel = self.viewModel else {
            // viewModel이 nil인 경우 처리 (예: 경고 메시지 표시)
            print("ViewModel is nil")
            return
        }

        let sheetViewController = CategorySettingsViewController(viewModel: viewModel) // 기존 ViewModel 인스턴스 사용
        ...
        if let sheet = sheetViewController.sheetPresentationController {
         ...
         NotificationCenter.default.addObserver(self, selector: #selector(handleCategoryEmptycheck), name: .categoryEmptycheck, object: nil)
