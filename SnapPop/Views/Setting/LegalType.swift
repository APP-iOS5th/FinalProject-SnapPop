...
enum LegalType: String {
    ...
    var content: String {
        ...
        case .privacyPolicy:
            return """
            SnapPop(이하 "앱")은 사용자의 개인정보 보호를 매우 중요하게 생각하며, 관련 법령을 준수하고 있습니다. 당사의 개인정보 취급방침은 사용자가 제공하는 개인정보가 어떤 목적으로 사용되는지, 어떻게 보호되고 관리되는지를 명확히 설명합니다.
            
            1. 개인정보의 수집 및 이용 목적
            앱에서는 사용자의 편의를 위해 애플 로그인과 구글 로그인을 허용합니다. 이를 통해 수집된 개인정보는 다음과 같은 목적으로 이용됩니다:
            - 사용자 인증 및 계정 관리
            - 서비스 제공 및 개선
            - 사용자 문의 및 요청에 대한 대응
            - 서비스 이용 통계 및 분석
            ...
            이 정책은 2024년 9월 6일부터 시행됩니다.
            """
        case .dataUsagePolicy:
            return """
            1. 데이터 수집 목적
            앱은 사용자의 편리한 서비스 이용을 위해 다양한 데이터를 수집합니다. 수집된 데이터는 다음과 같은 목적으로 사용됩니다:
            - 서비스 제공 및 개선
            - 사용자 경험 개인화
            - 서비스 관련 정보 제공
            - 고객 지원 및 문의 응답
            ...
            5. 데이터 공유 및 제공
            앱은 원칙적으로 사용자의 데이터를 외부에 제공하지 않습니다. 다만, 다음의 경우에는 예외로 합니다:
            - 사용자가 사전에 동의한 경우
            - 법령에 의해 요구되는 경우
            - 서비스 제공을 위해 필요한 경우 (예: 서버 호스팅 업체 등)
            
            6. 데이터 보안
            앱은 사용자의 데이터를 안전하게 보호하기 위해 다음과 같은 조치를 취하고 있습니다:
            - 데이터 암호화
            - 접근 통제 및 인증 절차
            - 정기적인 보안 점검 및 개선
            """
        case .termsOfService:
            return """
            제 1 조 (목적)
            본 약관은 SnapPop(이하 "앱")이 제공하는 모든 서비스(이하 "서비스")의 이용과 관련하여 앱과 이용자의 권리, 의무 및 책임사항, 기타 필요한 사항을 규정함을 목적으로 합니다.
