...
//        button.addTarget(self, action: #selector(detailButtonTapped(_:)), for: .touchUpInside)
    ...
    private func setupPieChartView() {
        ...
        NSLayoutConstraint.activate([
            ...
            pieChartView.topAnchor.constraint(equalTo: monthLabel.bottomAnchor, constant: 20),
            ...
            detailButton.topAnchor.constraint(equalTo: pieChartView.bottomAnchor, constant: 20),
            detailButton.trailingAnchor.constraint(equalTo: view.safeAreaLayoutGuide.trailingAnchor, constant: -30)
        ])
    ...
    private func setupChartView() {
        ...
        pieChartView.entryLabelColor = .black
          ...
          pieChartView.drawEntryLabelsEnabled = false // 라벨을 슬라이스 밖에 표시하지 않음
      }
    ...
    private func setData() {
        let entries = [
            PieChartDataEntry(value: 25, label: "카테고리1"),
            PieChartDataEntry(value: 25, label: "카테고리2"),
            PieChartDataEntry(value: 25, label: "카테고리3"),
            PieChartDataEntry(value: 15, label: "카테고리4"),
            PieChartDataEntry(value: 10, label: "카테고리5")
        ]

        let set = PieChartDataSet(entries: entries, label: "")
        set.colors = [.red, .green, .blue, .orange, .systemYellow]
        set.sliceSpace = 2
    ...
    // 나머지 함수들은 그대로 유지...
    func updateChartData(_ newData: [(value: Double, label: String, color: UIColor)], totalCost: String) {
            let entries = newData.map { PieChartDataEntry(value: $0.value, label: $0.label) }
            let set = PieChartDataSet(entries: entries, label: "")
            set.colors = newData.map { $0.color }
            set.sliceSpace = 2
            set.valueLinePart1OffsetPercentage = 0.8
            set.valueLinePart1Length = 0.2
            set.valueLinePart2Length = 0.4
            ...
            // DefaultValueFormatter를 수정된 방식으로 사용
            let formatter = DefaultValueFormatter(decimals: 1)
        formatter.formatter?.percentSymbol = "%"
